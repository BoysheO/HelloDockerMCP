...
public sealed class DockerContainerService
{
    ...
    public DockerContainerService(DockerGuard guard, StorageService storage, DockerImageService images)
    {
        _guard = guard;
        _storage = storage;
        _images = images;
    ...
    public async Task<object> RunDockerfileContainerAsync(
        string? dockerfile,
        string? imageTag,
        string? name,
        JsonElement? command,
        ...
        bool autoRemove)
    {
        var tag = string.IsNullOrWhiteSpace(imageTag) ? $"mcp-dockerfile:{Guid.NewGuid():N}" : imageTag.Trim();
        try
        {
            if (string.IsNullOrWhiteSpace(dockerfile))
            {
                return Error("MISSING_REQUIRED_ARGUMENT", "dockerfile is required.", "Pass the full single-file Dockerfile text.");
            }
            ...
            {
                runOk = document.RootElement.TryGetProperty("ok", out var okElement) && okElement.GetBoolean();
            }
        ...
        {
            return ToErrorResult(ex, image: tag);
        }
    }

    public async Task<object> RunContainersFromComposeYamlAsync(
        string? composeYaml,
        string? projectName,
        string? serviceName,
        long memoryMb,
        double cpus)
    {
        try
        {
            var createResult = await CreateContainersFromComposeYamlAsync(composeYaml, projectName, serviceName, memoryMb, cpus);
            ...
            if (root.TryGetProperty("created", out var createdElement) && createdElement.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in createdElement.EnumerateArray())
                {
                    ...
                    var startResult = await StartContainerAsync(id);
                    started.Add(startResult);
                }
            }
            ...
            return new
            {
                ...
                started,
                count = started.Count,
                note = "Created and started from Docker Compose yaml."
            };
        }
        catch (Exception ex)
        {
    ...
    public async Task<object> RemoveContainersAsync(IReadOnlyList<string>? containers, bool force = false)
    {
        try
        {
            if (containers is null || containers.Count == 0)
            {
                return Error("MISSING_REQUIRED_ARGUMENT", "At least one container identifier is required.", "Pass containers as an array of names or ids.");
            }
            ...
            foreach (var container in containers)
            {
                try
                {
                    var result = await RemoveContainerAsync(container, force);
                    removed.Add(result);
                }
                catch (Exception ex)
                {
                    warnings.Add(new { container, message = ex.Message });
                }
            }
    ...
    private string ValidateImage(string? image)
    {
        if (string.IsNullOrWhiteSpace(image))
        {
            throw new DockerToolException(
                ...
                "Use a full image reference from a trusted registry, preferably with a specific version tag instead of latest.",
                new { trustedRegistries = _guard.TrustedRegistryList });
        }
